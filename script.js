...
formulaBar.addEventListener("blur", function () {
    ...
    // If formula is cleared, remove it from the cell
    ...
    // If formula changes, remove old one and add the new one
...
function addFormula(formula) {
    let obj = getCellobj(lsc);
    // Split the formula by spaces to extract cell references
    let formulaArray = formula.split(" ");
    let children = obj.child;

    // Check for circular references in child cells
    ...
    // Check for circular reference with the formula itself
    for (let j = 0; j < formulaArray.length; j++) {
    ...
    // Assign the formula to the cell and solve it
    obj.formula = formula;
    solveFormula(obj);
    return true;
}
...
function removeFormula(obj) {
    ...
    // Remove the cell from the parent's child list
...
function showErr(obj) {
    ...
    // Show errors for child cells as well
    for (let i = 0; i < obj.child.length; i++) {
